...
- **Global scope**: `/home/user/SavoryMind/slotly/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/user/SavoryMind/slotly/.claude/notes/`)
...
2. Glob `/home/user/SavoryMind/slotly/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/user/SavoryMind/slotly/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/user/SavoryMind/slotly/.claude/notes/` — works in any directory
