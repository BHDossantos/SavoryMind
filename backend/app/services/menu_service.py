...
def get_all_items(db: Session) -> list[MenuItem]:
...
def get_dashboard_stats(db: Session) -> DashboardStats:
    ...
    total_revenue = sum(i.price * i.orders_last_30_days for i in items)
    margins = [((i.price - i.cost) / i.price) * 100 for i in items if i.price > 0]
...
def get_recommendations(db: Session) -> list[dict]:
    ...
    for item in items:
        ...
        elif margin > 60 and item.orders_last_30_days < 20:
            recommendations.append({
                "item": item.name,
                "category": item.category,
                "type": "promotion",
        ...
        elif revenue > 2000 and margin > 50:
            recommendations.append({
                "item": item.name,
                "category": item.category,
                "type": "star_item",
