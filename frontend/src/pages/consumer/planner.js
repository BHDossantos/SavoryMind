...
const DIETARY_DEFS = [
  { value: "",           labelKey: "plannerPage.dietAll" },
  { value: "vegetarian", labelKey: "plannerPage.dietVegetarian" },
  { value: "vegan",      labelKey: "plannerPage.dietVegan" },
  { value: "keto",       labelKey: "plannerPage.dietKeto" },
  { value: "gluten_free",labelKey: "plannerPage.dietGlutenFree" },
  { value: "dairy_free", labelKey: "plannerPage.dietDairyFree" },
];
...
function PlannerPageInner() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">{t("plannerPage.title")}</h1>
        <p className="text-gray-400 mt-1">{t("plannerPage.subtitle")}</p>
      </div>
      ...
      {suggestion && (
        <div className="bg-consumer-50 border border-consumer-200 rounded-2xl p-5 mb-6">
          <p className="text-xs font-semibold text-consumer-600 uppercase tracking-wide mb-1">
            {t("plannerPage.todayPrefix")} {suggestion.day}
          </p>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        <span className="text-xs font-semibold text-gray-500 self-center mr-1">{t("plannerPage.dietLabel")}</span>
        {DIETARY_DEFS.map((opt) => (
      ...
      <div className="flex border-b border-gray-200 mb-6">
        {[["plan", t("plannerPage.tabWeekly")], ["shopping", t("plannerPage.tabShopping")]].map(([key, label]) => (
          ...
          {plan.days.map((day) => (
            <div key={day.day} className="bg-white rounded-2xl border border-consumer-100 overflow-hidden">
              ...
              <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                <MealCard label={t("plannerPage.mealLunch")} meal={day.lunch} />
                <MealCard label={t("plannerPage.mealDinner")} meal={day.dinner} />
              </div>
            </div>
          ))}
        </div>
      ...
      {tab === "shopping" && shopping && (
        <div>
          <p className="text-sm text-gray-500 mb-4">{t("plannerPage.shoppingHeader", { count: shopping.total_items })}</p>
