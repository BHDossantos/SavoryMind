...
const MOODS = ["cozy", "romantic", "light", "adventurous", "indulgent", "healthy", "brunch", "group", "quick"];
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-gray-400 mt-1">AI-curated recipes matched to your mood, cuisine, and craving</p>
      </div>

      {/* Filters */}
      <div className="bg-consumer-50 rounded-2xl p-5 mb-6 space-y-4">
        <div className="flex flex-wrap gap-2">
          <span className="text-xs font-semibold text-gray-500 self-center mr-1">Mood:</span>
          {MOODS.map((m) => (
            <button key={m} onClick={() => { setMood(mood === m ? "" : m); }}
              className={`text-xs px-3 py-1.5 rounded-full font-medium transition-all ${mood === m ? "bg-consumer-600 text-white" : "bg-white text-gray-600 border border-consumer-200 hover:border-consumer-400"}`}>
              {m}
            </button>
          ))}
        </div>
      ...
      {selected && (
        <div>
          <button onClick={() => setSelected(null)} className="text-sm text-consumer-600 hover:underline mb-6">← Back to results</button>
            ...
            <div className="flex items-start gap-6 mb-6">
              ...
              <div className="flex-1">
                ...
                <div className="flex gap-3 flex-wrap">
                  ...
                  <span className="text-xs bg-gray-100 text-gray-700 px-3 py-1 rounded-full">👥 Serves {selected.servings}</span>
