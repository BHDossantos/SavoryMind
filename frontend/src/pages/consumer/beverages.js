...
const TAB_LABELS = { wine: "Wine", beer: "Beer", spirits: "Spirits" };
...
export default function Beverages() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">🥂 Beverage Pairing</h1>
        <p className="text-gray-400 mt-1">AI-matched wine, beer & spirits for any dish</p>
      </div>
      ...
      <div className="flex gap-2 mb-6 bg-consumer-50 rounded-2xl p-1.5 w-fit">
        {TABS.map((t) => (
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all ${tab === t ? "bg-white shadow-sm text-consumer-700" : "text-gray-500 hover:text-consumer-600"}`}
          >
      ...
      <form onSubmit={handleSearch} className="flex gap-3 mb-8 max-w-xl">
        <input
          ...
          placeholder={`Enter a dish to pair with ${TAB_LABELS[tab].toLowerCase()}...`}
      ...
      {pairings.length > 0 && (
        <div>
          <h2 className="font-semibold text-gray-700 mb-4">
            Top {TAB_LABELS[tab]} pairings for <span className="text-consumer-700">"{results.dish || results.dish_name}"</span>
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {pairings.map((p, i) => (
              <div key={i} className={`bg-white rounded-2xl border p-5 shadow-sm ${i === 0 ? "border-consumer-300 ring-2 ring-consumer-100" : "border-consumer-100"}`}>
                {i === 0 && <div className="text-xs font-bold text-consumer-600 mb-2 uppercase tracking-wide">⭐ Top Match</div>}
                ...
                {tab === "beer" && (
                  <>
                    <h3 className="font-bold text-gray-900">{p.name}</h3>
                    <p className="text-xs text-gray-500">{p.style} · {p.abv}% ABV</p>
      ...
      {!results && (
        <div className="mt-4">
          <p className="text-xs text-gray-400 mb-3">Try these:</p>
